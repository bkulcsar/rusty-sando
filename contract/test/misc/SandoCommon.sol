...
library FiveBytesEncodingUtils {
    ...
    function encode(
        uint256 amount
    ) public pure returns (EncodingMetaData memory encodingParams) {
     ...
     */
    function decode(
        EncodingMetaData memory params
    ) public pure returns (uint256 decodedValue) {
        decodedValue =
            uint256(params.fourBytes) <<
            (uint256(params.byteShift) * 8);
    }
     ...
     */
    function finalzeForParamIndex(
        EncodingMetaData calldata encodingParams,
        uint8 paramIndex
    ) public pure returns (uint40 fiveBytes) {
        // 4 for function selector
        uint8 memLocation = 4 +
            32 +
            (paramIndex * 32) -
            4 -
            encodingParams.byteShift;

        bytes memory encodedBytes = abi.encodePacked(
            memLocation,
            encodingParams.fourBytes
        );
        assembly {
...
library SandoCommon {
    ...
    function getJumpDestFromSig(
        ...
        for (uint256 i = 0; i < functionNames.length; i++) {
            if (
                keccak256(abi.encodePacked(functionNames[i])) ==
